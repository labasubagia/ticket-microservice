...
const run = async (): Promise<void> => {
  ...
  const proms = Array.from({ length: 5 }).map(async (_v, idx) => {
    count++
    return await publisher1.publish({
      id: String(count),
      title: `created ${count}`,
      price: 200,
  ...
  const proms2 = Array.from({ length: 5 }).map(async (_v, idx) => {
    count++
    return await publisher2.publish({
      id: String(count),
      title: `updated ${count}`,
      price: 50,
      userId: randomBytes(5).toString('hex')
